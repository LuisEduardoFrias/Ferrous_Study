...
export const githubServiceApi = {
   ...
   async getFileContentByMarkdown(fileName: string): Promise<TMarkdownResult | null> {
      try {
         ...
         if (!response.ok) {
            console.error(`Error al obtener el contenido de ${fileName}:`, response.status);
            return null;
         }
         ...
         console.error(`Error de red al obtener el contenido de ${fileName}:`, error);
         return null;
      }
   },
   ...
   async login(username: string, password: string): Promise<TUser | { error: string }> {
      try {
   ...
   async sendEmail(suggestion: TSuggestion): Promise<boolean> {
      try {
   ...
   async createMarkdownFile(fileName: string, content: string, keywords: string[]): Promise<TServiceResult | null> {
   ...
   async updateFileContent(fileName: string, content: string | TMenu[], type: typeFile): Promise<TServiceResult> {
      ...
      if (!token) {
         console.error('No hay token de autenticación disponible.');
         return { message: 'No hay token de autenticación disponible.' };
      }
...
function saveTokenToCookie(token: string) {
   Cookies.set(TOKEN_COOKIE_NAME, token, { expires: 1, secure: NODE_ENV === 'production', sameSite: 'Lax' });
}
